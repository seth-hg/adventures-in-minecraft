...
玩minecraft的时候，你玩的是一个别人设计好的游戏。尽管minecraft世界很有趣，但是如果能让它按照你的想法来做事，那会更有趣一点。用python进行minecraft编程的时候，你的指尖上有一个完整的编程环境，你可以发明任何你能想象到的东西并通过编程来实现。这个冒险任务将教会你一些minecraft和python编程语言可以做的有趣的事情。

通过编程可以做的一件很有趣的事情是创造游戏中的游戏。minecraft就是世界，你的程序可以让这个世界按照新的方式运转。你将会发现大多数minecraft程序都包含三个有趣且让人激动的东西：*感知*这个世界，例如人物的位置；*计算*一些新的东西，比如某种分数；以及做出某些行为，比如移动你的角色到新的位置。

在这个冒险任务里，你会学到如何感知角色的位置并随着角色的移动来出发不同的事情。在欢迎回家这个游戏里，你会创造一个踏上去会跟你打招呼的神奇门垫。我们用真正的minecraft围栏来介绍一种称作“地理围栏(geo-fencing)”的技术。你和你的朋友们需要用最快的速度来收集物品以完成游戏的挑战。最后，如果你不能快速离开那个区域的话，你会学到通过将角色投射到空中来移动角色的方法。

>程序是你用某种编程语言编写的一组指令或者语句，计算机将会自动执行这些指令。你必须使用你的编程语言中正确的指令。在本书中，你会使用python编程语言。

>语句是一个通用计算机术语，通常是指你给计算机的一个完整的指令，比如像print("hello steve")这样的一行计算机程序。有些人也用“命令”这个词，不过那通常是指你在命令提示符后输入给电脑的命令。
>Python对于什么是语句有自己的精确解释，不过对于这本书来讲，我会用语句这个词来表示python程序中的一条指令或者一行。如果想了解更多python的知识，可以在这里阅读在线文档：https://docs.python.org/2/

##感知角色的位置

在感知角色的位置之前，你需要理解一下minecraft的世界是如何组织的。minecraft中每样东西的大小都是一个方块，一个方块表示一立方米。你的角色Steve在minecraft世界中移动的时候，游戏会用一组座标来记录他的位置。游戏世界是通过在空方块里填充各种一立方米体积的材料而构建起来的。有一些情况例外，比如地毯和水、岩浆之类的液体看起来可能不足一立方米，但是也不能和其他材料放在一个方块，即使他们体积加起来也不到一立方米。

通过感知角色的位置，你可以让你的游戏随着你的移动而作出只能的反应，比如在特定的位置现实某些信息，在移动过程中自动建造建筑，走到特定地点自动改变角色的位置。这些在这本书里都可以学到，其中很多你甚至在这个冒险任务里就可以学到。

>座标是一组表示唯一位置的数字。minecraft使用3D座标来表示三维世界中的准确位置，每组座标包含三个数字。在这里https://www.evernote.com/shard/s64/nl/1831858487/cb669c86-4753-4ca5-b79d-125cee34a941/了解更多关于座标的知识，这里http://minecraft.gamepedia.com/Coordinates学习在minecraft世界里如何使用座标。

这些座标被称作x、y和z。在minecraft里提到座标的时候需要比较小心；如果你使用“左边”或者“右边”这样的词汇，左边还是右边取决于你面朝那个方向。把minecraft的座标类比为指南针的方向会比较容易理解。图2-1展示了座标x、y和z的改变跟运动方向的关系。

>自己在游戏里试验一下会更容易理解角色移动的时候座标是如何改变的。在树莓派上，当你移动角色时，x、y和z座标会现实在屏幕左上角。在PC/苹果电脑上你可以按F3键来现实座标。在这里你可以学到minecraft的所有高级控制功能：http://minecraft.gamepedia.com/Coordinates
...
既然已经完成了冒险任务1里的hello minecraft world程序，是时候来编写你自己的程序来连接Minecraft了！

冒险任务1详细解释了怎样让所有东西运行起来，随着冒险任务的进行你会越来越熟悉这些步骤。启动所有程序并开始输入代码需要的所有步骤都在冒险任务1里介绍了，不过在最初的几个冒险任务中，因为你还在学习怎样操作，我们会给出一些提示。

>想要关于如何在你的电脑上运行minecraft的提示，请访问配套网站www.wiley.com/go/adventuresinminecraft，选择冒险任务1 视频。

1. 启动minecraft，idle，如果在pc或者mac上运行，也要启动服务器。你应该已经练习过如何启动所有程序，需要提示的话请复习冒险任务1。
2. 打开idle，python集成开发环境，就像在冒险任务1里那样。这是你进入python编程语言的入口，也是输入和运行所有python程序的地方。

##显示角色的位置

理解坐标最好的办法是写一个小的试验性程序，这就是你现在要做的。这个程序会显示游戏角色的3D坐标，并且你可以看到坐标随着角色的移动而改变。

1. 在python shell里，选择file->new file新建一个程序。这会打开一个新的未命名窗口，是你输入新程序的地方。
2. 输入程序之前，先保存文件是一个好习惯，这样如果你急需离开电脑的时候，就不需要在压力之下想出一个文件名了。只需要选择保存即可。专业程序员总是这样做来保证不会丢掉自己的工作成果。在python shell菜单里，选择file->save as，输入文件名whereAmI.py。请确保文件保存在MyAdventures文件夹里。
3. 你的程序将通过minecraft api直接和minecraft游戏进行通信。要调用这些API，首先要导入minecraft模块，它让你的程序能访问minecraft游戏的所有设施。用下面语句导入minecraft模块。
...
要让知道你的角色是否在神奇门垫上，你的程序必须至少检查座标的两个部分。检查门垫的x和z座标是否跟角色的位置相同，这看起来是一种检查角色是否站在门垫上的合理方法。你可以在if语句中用and关键词来检查两个条件是不是同时成立。y座标在这里不是很重要。因为这里不检查y座标，所以如果角色悬浮在门垫上方，你也一样会看到欢迎回家的消息。
...
2. 要知道门垫的座标，再次运行你的whereAmI.py程序，然后站在门垫上。记下x、y和z座标，因为你编写新程序的时候会用它们来确定门垫在minecraft世界里的位置。
...
6. 添加if语句检查角色是否站在门垫上。这里要使用带有and的if语句来同时检查两个条件。门垫的x和z座标必须同时和角色的位置一致才能判断角色正站在门垫上。还记得你之前得到的x和z座标吗？写在这里让程序知道你的门垫在什么位置：
...
你觉得检查y座标（上下方向）能改进对角色是否站在门垫上的检测吗？试一下。修改程序来检测门垫的全部三个座标，看看是不是工作得更好。
...
程序welcomeHome.py使用if语句来检查角色的座标是否跟门垫相同。要触发这个程序，你的角色必须刚好站在门垫上。你可以用地理围栏技术来进行一些改进，你在这个新游戏里也会用到这个技术。对角色位置的检测有所改进，因为它可以检查你的角色是否站在minecraft世界的某个区域中，而不是某个特定的座标。这样你就可以判断角色是否站在一大片区域中的任意位置，而不需要精确的检查每个方块。

>地理围栏是一种通用技术，可以在任何地图上的指定座标附近建立虚拟的围栏。当有东西进入虚拟围栏包围的区域，就会产生某些结果。这里的“东西”可以是minecraft世界中的角色、现实世界的人、或者是割草机之类的设备，甚至是现实世界的动物。
>
...
实现地理围栏需要两样东西：一个需要跟踪的物体或者区域，和围栏为这个物体所圈定的座标范围。你接下来要建造一块用minecraft中的真实围栏所包围的土地，并且记下这块地四个角落的座标。
...
像这样列出你的最小和最大座标，你在下一个步骤——编写程序——里会泳道它们。

###编写地理围栏程序

最后一步是编写程序实现地理围栏并在角色站到指定区域中时收取租金。这个程序在结构上跟whereAmI.py相似。

在这个新游戏里还会用到**常量**，这让你以后可以很容易的移动划定的区域。用这种方式，如果要改变区域的位置，就不需要到程序代码中去四处寻找需要修改的座标值了。
...
5. 现在，你需要定义常量来表示围栏的4个座标。确保你使用的是之前得到的座标。这里列出的是我在自己的程序里使用的座标，你的可能有所不同：
...
8. 判定角色是否在围栏圈定的区域之前，需要先获得角色的座标。就像在之前的程序中一样，用player.getTilePos()来实现：

		pos = mc.player.getTilePos()

9. 现在到了程序最重要的部分。这里你要让程序使用之前获得的四个座标来判断角色是否在围栏区域内，如果是，就要收取租金并且向聊天栏发送一条消息通知它。输入下面代码：
